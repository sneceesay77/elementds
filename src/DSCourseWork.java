			...
			while (in.hasNextLine()) {
				...
				if (linecount > 1) {
					...
					if(keys.size() == 1 || itrCount == 0){
						...
						createTable.append(keys.get(0)+" TEXT PRIMARY KEY NOT NULL,");
						itrCount++;
					...
					for (int i = 1; i < tokens.length; i++) {
						...
						if (i % 2 != 0) {//odd positions have the fields
							//If the field is not a key and not a NIL mapping then append it to create 
							//table string.
							if((!keys.contains(tokens[i])) && !tokens[i+1].equals("nil")){
								createTable.append(tokens[i] + " TEXT  NOT NULL,\n");
			...
			while(in.hasNextLine()){
				...
				for(int i=1; i<tokens.length; i++){
						...
						if(i%2!=0){
							...
							if(tokens[0].equalsIgnoreCase("Quote") || tokens[0].equalsIgnoreCase("RFQ")){
								if(tokens[i].equalsIgnoreCase("rid")||tokens[i].equalsIgnoreCase("qid")){currId=tokens[i+1];}
                ...
                "VALUES ("+removeLastChar(values)+");"; 
				 ...
				 if(tokens[0].equalsIgnoreCase("Quote")){ 
					 ...
					 while(rs.next()){
						 ...
						 if(!item.equalsIgnoreCase(currItem)){
							 integrityMsg = "FAILS integrity, the key rid with value "+currId+" is bound to "+item+" as an item."; 
						 }
					 }
				 ...
				 if(tokens[0].equalsIgnoreCase("RFQ")){ 
					 String sqlQuery = "SELECT item FROM Quote WHERE rid = '"+currId+"'";
					 st = ct.createStatement();
					 ResultSet rs = st.executeQuery(sqlQuery);
					 while(rs.next()){
						 String item = rs.getString("item");
						 if(!item.equalsIgnoreCase(currItem)){
							 integrityMsg = "FAILS integrity, the key qid with value "+currId+" is bound to "+item+" as an item."; 
						 }
					 }
